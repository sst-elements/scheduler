...
RoundUpMBSAllocator::RoundUpMBSAllocator(vector <string> *params, StencilMachine *mach)
    : GranularMBSAllocator(params, mach) {
...
map<int, int> *RoundUpMBSAllocator::generateIdealRequest(int procs) {
    ...
    if (ordering->at(roundedIndex) < procs) {
        retVal->insert(pair<int, int>(roundedIndex,
                                      (int) ceil(((double) procs) /
                                                 ((double) ordering->at(roundedIndex)))));
    } else {
    ...
    while (RBR->size() > 0) {
        ...
        if (numberAvailable == 0) {
            ...
            if (splitLarger(key)) {
                ...
                for (map<int, int>::iterator entry = reduced->begin();
                     entry != reduced->end(); entry++) {
        ...
        if (numberAvailable > 0) {
            ...
            for (int assigned = 0; assigned < toAssign && value > 0; assigned++) {
                vector < Block * > *toAllocate = makeBlockAllocation(*(FBR->at(key)->begin()),
                                                                     nodesNeeded - procs);
...
vector<Block *> *RoundUpMBSAllocator::partiallyAllocate(Block *block, int procs) {
    ...
    int blockRank = distance(ordering->begin(),
                             find(ordering->begin(), ordering->end(), (block->size())));
    ...
    while (children != block->children->end()) {
        Block *next = *children;
        children++;
        int nextRank = distance(ordering->begin(),
                                find(ordering->begin(), ordering->end(), next->size()));
 ...
 */
MBSMeshAllocInfo *RoundUpMBSAllocator::allocateBlocks(MBSMeshAllocInfo *retVal,
                                                      vector<Block *> *blocks, int allocated) {
...
map<int, int> *RoundUpMBSAllocator::reduceRequest(int key, int value) {
    ...
    if (remainder > 0) {
        int smallerIndex = distance(ordering->begin(),
                                    find(ordering->begin(), ordering->end(), remainder));
        if (smallerIndex == -1) {
