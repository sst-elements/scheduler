...
Mesh3DMachine::Mesh3DMachine(std::vector<int> dims, int numCoresPerNode, double **D_matrix)
    : StencilMachine(dims,
                     3 * dims[0] * dims[1] * dims[2] - dims[0] * dims[1] - dims[0] * dims[2] -
                     dims[1] * dims[2],
                     numCoresPerNode,
                     D_matrix) {
...
std::list<int> *Mesh3DMachine::getFreeAtDistance(int center, int dist) const {
    ...
    for (curDims[0] = std::max(centerX - dist, 0);
         curDims[0] <= std::min(centerX + dist, dims[0] - 1); curDims[0]++) {
        int yRange = dist - abs(centerX - curDims[0]);
        for (curDims[1] = std::max(centerY - yRange, 0);
             curDims[1] <= std::min(centerY + yRange, dims[1] - 1); curDims[1]++) {
...
void Mesh3DMachine::appendIfFree(std::vector<int> curDims, std::list<int> *nodeList) const {
    if (curDims[0] >= 0 && curDims[0] < dims[0] && curDims[1] >= 0 && curDims[1] < dims[1] &&
