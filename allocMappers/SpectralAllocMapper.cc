...
SpectralAllocMapper::SpectralAllocMapper(const Machine &mach, bool alloacateAndMap, int rngSeed)
    : AllocMapper(mach, alloacateAndMap) {
    /*   if(rngSeed > 0){
...
void SpectralAllocMapper::allocMap(const AllocInfo &ai,
    ...
    if (mach.coresPerNode != 1) {
        schedout.fatal(CALL_INFO, 1,
                       "Multi-core node support is not yet implemented for SpectralAllocMapper\n");
    }
    ...
    while (mappedcount > 0) {
        ...
        for (list<unsigned long>::const_iterator it = pairIndices.begin();
             it != pairIndices.end(); it++) {
        ...
        for (list<unsigned long>::iterator it = pairIndices.begin();
             it != pairIndices.end(); it++) {
...
vector<double> *SpectralAllocMapper::multWithM(const vector<double> &inVector) const {
    ...
    for (list<unsigned long>::const_iterator i = pairIndices.begin(); i != pairIndices.end(); i++) {
        ...
        for (list<unsigned long>::const_iterator j = pairIndices.begin();
             j != pairIndices.end(); j++) {
