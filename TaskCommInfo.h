...
namespace SST {
    namespace Scheduler {
        ...
        public:
            ...
            TaskCommInfo(Job *job, std::vector <std::map<int, int>> *inCommInfo,
                         int centerTask = -1); // communication matrix input
            TaskCommInfo(Job *job, int xdim, int ydim, int zdim,
                         int centerTask = -1); // mesh dimension input
            TaskCommInfo(Job *job, std::vector <std::map<int, int>> *inCommInfo, double **inCoords,
                         int centerTask = -1); //coordinate input
        ...
        private:
            ...
            void getTaskDims(int taskNo,
                             int outDims[3]) const; //returns task position for mesh structure
        };
    }
}
#endif
